...
import { jdnToEthiopic, gregorianToJDN } from "./utils";
...
export class EthiopianCalendar {
  ...
  public toEthiopian(dateArray: number[] | number | string, month?: number, date?: number) {
    ...
    let jdn = gregorianToJDN(year, gregorianMonth, gregorianDate);
    const [ethiopianYear, ethiopianMonth, ethiopianDate] = jdnToEthiopic(jdn);
